...
import numpy as np
from preprocess2 import get_cis_data
import torch
from util2 import *
...
def main(args):
    X_train, X_test, y_train, y_test, num_names, cat_names, final_names = get_cis_data(seed = 2000)
    kde = KernelDensity(kernel='gaussian', bandwidth=0.5).fit(X_train[0:30000])
    aa = (kde.score_samples(X_test[0:300]))

    bins = np.arange(np.min(aa) + 350, np.max(aa), 15)
    ...
    if args.svd == 'load':
        s = np.load('./svd_logs/s_cis.npy')
        Vh = np.load('./svd_logs/Vh_cis.npy')
    elif args.svd == 'mixed':
        c, s, Vh = svd_mixed(X_train, len(num_names), dataset= 'cis')
    else:
        raise ValueError
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--m-weight', type=float, help= 'M-Distance Penalty', default= 0.0)
    args = argparser.parse_args()
    print('IEEE CIS Fraud')
    print(args)
    main(args)