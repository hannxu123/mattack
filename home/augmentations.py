...
class RegressionSMOTE():
    ...
    """
    def __init__(self, p=0.8, alpha=0.5, beta=0.5, seed=0):
        ""
        self.seed = seed
        self._set_seed()
        self.device = 'cuda'
...
class ClassificationSMOTE():
    ...
    """
    def __init__(self, device_name="auto", p=0.5, alpha=0.5, beta=0.5, seed=0):
        ""
        self.seed = seed
        self._set_seed()
    ...
    def __call__(self, X):
        ...
        X2 = np.copy(X)
        batch_size = X.shape[0]
        random_values = np.random.uniform(0,1,batch_size)
        idx_to_change = (random_values < self.p)

        # ensure that first element to switch has probability > 0.5
        np_betas = np.random.beta(self.alpha, self.beta, batch_size) / 2 + 0.5
        index_permute = np.random.permutation(batch_size)

        X2[idx_to_change] = np_betas[idx_to_change, None] * X2[idx_to_change] +\
         (1 - np_betas[idx_to_change, None]) * X2[index_permute][idx_to_change]  # noqa
