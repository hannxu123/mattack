...
from utils import get_gumbel_output2, get_gumbel_output3, get_ce_loss
import cvxpy as cp
...
def pgd_attack(model, x, y,
    ...
    for j in range(num_steps):
        ...
        loss = torch.mean(criterion(torch.sigmoid(output), y.repeat(500, 1))) - (np.log(1 + m_weight / 2)) * torch.mean(M_dist)
        ...
        z_cat = torch.clip(z_cat, min = 0, max = 10)
